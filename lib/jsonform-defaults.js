...
(function () {
  ...
  // In particular, the following line copies the global "define" method to a
  // local variable if it exists, and handles cases 2. and 3. otherwise
  var define = (typeof define !== "undefined") ? define : function (deps, func) {
    // "define" is not available, so not an AMD module.
    ...
    if (module.exports) {
      ...
      // No way to export as a node.js module either.
      ...
      root['JSONForm'].setDefaultValues = func().setDefaultValues;
    }
  };

  // Here, the call to define uses the local variable, meaning it results in
  // a call to the global define or to the function defined above that exports
  // the result as a node.js module or as a JSONForm.setDefaultValues method.
  define([], function () {
    /**
     * Sets default values for the datasource, ensuring that fields defined as
     * "required" appear in the object. If missing, the hierarchy that leads to
    ...
    // Return the setDefaultValues to the external world
    return {
