...
formFragment.prototype.buildOneElement = function(elt,options) {
  ...
  if (schema.items && schema.type=="array") {
    ...
    if (elt.type=="checkboxes" && schema.items["enum"]) {
      
      schema._jsonform_checkboxes_as_array=true;

      var vals = schema.items["enum"];

      elt.itemshtml="";
      _.each(vals,function(v,i) {
        // We should do a proper formFragment but bypassing it here seems to keep the code simple
        elt.itemshtml+=_.template(jsonform.elementTypes["checkboxesitem"].template)({
          "elt":{
            "value":_.include(elt.value,v),
            "title":elt.titleMap?elt.titleMap[v]:v,
            "name":elt.key+"["+i+"]"
          }
        });
      });
...
jsonform.getFormValue = function(formelt) {
  ...
  for (var i = 0; i < formArray.length; i++) {
    ...
    if (arrayMatch) {
      ...
      } else if (eltSchema._jsonform_checkboxes_as_array) {
