...
jsonform.defaultTemplates = {
  ...
  'array':   jsonform.defaultFieldTemplate('<div data-jsonform-elemcount="1" id="<%= elt.id %>"><%= elt.itemshtml %></div><a href="#" onclick="$(\'#<%= elt.id %>\').append(<%= escape(JSON.stringify(elt.itemshtmltpl)) %>.replace(/XXXkeyXXX/g,\'<%= elt.key %>[\'+$(\'#<%= elt.id %>\').data(\'jsonform-elemcount\')+\']\')); $(\'#<%= elt.id %>\').data(\'jsonform-elemcount\',parseInt($(\'#<%= elt.id %>\').data(\'jsonform-elemcount\'),10)+1); return false;">Add new</a>')
};
...
formFragment.prototype.buildOneElement = function(elt,options) {
  ...
  if (schema.items && schema.type=="array") {
    ...
    subForm.build({
      "schema":{
        "XXXkeyXXX":schema.items
      },
      "form":["XXXkeyXXX"]
    });
    console.log("subform",subForm.html);
    elt.itemshtmltpl = subForm.html;
    elt.itemshtml = elt.itemshtmltpl.replace(/XXXkeyXXX/g,elt.key+"[0]");
    subForm.html="";
    this.concat(subForm);
  }
