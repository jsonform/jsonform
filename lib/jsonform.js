...
var fileDisplayTemplate = '<div class="_jsonform-preview">' +
  ...
  '<% } %>' +
  '</div>' +
  '<a href="#" class="btn btn-light _jsonform-delete"><i class=" bi-trash" title="Remove"></i></a> ';
...
jsonform.elementTypes = {
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<div class="dropdown">' +
      '<a class="btn<% if (buttonClass && node.value) { %> <%= buttonClass %><% } else { %> btn-light<% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
      ...
      '<div class="dropdown-menu navbar" id="<%= node.id %>_dropdown">' +
        ...
        '</div>' +
        '<div class="pagination-right"><a class="btn btn-light">Reset</a></div>' +
      '</div>' +
      '</div>' +
      '</div>',
    'fieldtemplate': true,
  ...
  'radiobuttons': {
    'template': '<div id="<%= node.id %>">' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<label class="btn btn-light <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %>active btn-success<% } %>">' +
  ...
  'array': {
    'template': '<div id="<%= id %>"><ul class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>' +
      '<span class="_jsonform-array-buttons">' +
        '<a href="#" class="btn btn-light _jsonform-array-addmore"><i class="bi-plus-circle" title="Add new"></i></a> ' +
        '<a href="#" class="btn btn-light _jsonform-array-deletelast"><i class="bi-trash" title="Delete last"></i></a>' +
      '</span>' +
      '</div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner, enableDrag) {
      if ($('').sortable) {
        ...
        return '<li data-idx="<%= node.childPos %>">' +
          ...
          (enableDrag ? '<span class="draggable line"><i class="bi-list" title="Move item"></i></span>' : '') +
          inner +
          '</li>';
      }
      else {
  ...
  'tabarray': {
    'template': '<div id="<%= id %>"><div class="tabbable tabs-left">' +
      ...
      '</div>' +
      '<a href="#" class="btn btn-light _jsonform-array-addmore"><i class="bi-plus-circle" title="Add new"></i></a> ' +
      '<a href="#" class="btn btn-light _jsonform-array-deleteitem"><i class="bi-trash" title="Delete item"></i></a></div>',
    'fieldtemplate': true,
    'array': true,
  ...
  'button':{
    'template':' <button type="button" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-light <%= elt.htmlClass?elt.htmlClass:"" %>"><%= node.title %></button> '
  },
  'actions':{
   ...
   */
  'question': {
    'template': '<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><label class="<%= (node.formElement.optionsType === "radiobuttons") ? "btn btn-light" : "" %><%= ((key instanceof Object && key.htmlClass) ? " " + key.htmlClass : "") %>"><input type="radio" <% if (node.formElement.optionsType === "radiobuttons") { %> style="position:absolute;left:-9999px;" <% } %>name="<%= node.id %>" value="<%= val %>"<%= (node.disabled? " disabled" : "")%>/><span><%= (key instanceof Object ? key.title : key) %></span></label> <% }); %></div>',
    'fieldtemplate': true,
    'onInsert': function (evt, node) {
      var activeClass = 'active';
      var elt = node.formElement || {};
