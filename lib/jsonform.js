...
jsonform.defaultTemplates = {
  ...
  'file-transloadit': jsonform.defaultFieldTemplate('<input type="hidden" name="params" value=\'<%= escape(elt.transloaditParams) %>\'><input type="file" name="transloaditfile-<%= elt.name %>" />'), /*<input type="hidden" id="<%= elt.id %>" name="<%= elt.name %>" />*/
...
formFragment.prototype.concat = function(form2) {
  ...
  this.eventhandlers = this.eventhandlers.concat(form2);
};
*/
...
formFragment.prototype.buildOneElement = function(elt,options) {
  ...
  if (elt.items) {
    ...
    this.eventhandlers = this.eventhandlers.concat(subForm.eventhandlers);
  }
