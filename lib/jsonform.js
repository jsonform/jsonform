...
jsonform.elementTypes = {
  ...
  'array': {
    ...
    'onInsert': function (evt, node) {
      var $nodeid = $('#' + escapeSelector(node.id));
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      ...
      // Find the first array key based on the key of the first element in the tabarray items
      var firstArrayKey = node.children[0].children[0].key.replace(/\[0\][^\[]*$/,"");
      var firstArraySchema = getSchemaKey(node.ownerTree.formDesc.schema.properties, firstArrayKey);

      var maxElements = -1;
      if (firstArraySchema && firstArraySchema.maxLength) {
        maxElements = firstArraySchema.maxLength;
      }
      ...
      $('> a._jsonform-array-deleteitem', $nodeid).click(function (evt) {
        ...
        node.deleteArrayItem(idx);
        updateTabs();
        if (maxElements>=0 && idx<=maxElements-1) {
      ...
      $('> a._jsonform-array-addmore', $nodeid).click(function (evt) {
        var idx = node.children.length;
        if (maxElements>=0) {
          if (idx>maxElements-2) $('> a._jsonform-array-addmore', $nodeif).addClass("disabled");
          if (idx>maxElements-1) return false;
        }
        evt.preventDefault();
        evt.stopPropagation();
...
/**
 * Helper function that returns an element type definition that works for both the JSON Schema official
 * 'utc-millisec' format and the non-standard but more DB-compatible 'utc-sec'
 * @param precisionAdjustment
 * @return {Object}
 */
var elementTypeUtcVariant = function(precisionAdjustment) {
  return {
    'template':'<input class="input-xlarge xlarge" type="text" name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>" />',
    'fieldtemplate': true,
    'inputfield': true,
    'onInsert': function(evt, node) {
      // Clone the real input field
      var $input = $(node.el).find('input');
      var $editor = $input.clone().attr({
        name: $input.attr('name') + '-datetimepicker',
        id: $input.attr('id') + '-datetimepicker'
      });

      // Convert the time format whenever the user selects a different time
      $editor.on('change', function() {
        // Convert to UTC, with the right precision
        $input.val(Date.parse($editor.val()) / precisionAdjustment);
      });

      // Insert the pop-up date-time picker
      $editor.datetimepicker({
        ampm: true,
        showTimezone: true,
        // useLocalTimezone:true automatically changes the timezone according to DST when the user scrolls through the months
        useLocalTimezone: true,
        dateFormat: 'd M yy',
        timeFormat: 'h:mm TT z'
      });
      $("#" + escapeSelector(node.id)).hide().after($editor);
    }
  };
}

jsonform.elementTypes['utc-millisec'] = elementTypeUtcVariant(1);
jsonform.elementTypes['utc-sec']      = elementTypeUtcVariant(1000);
 ...
 */
var getInitialValue = function (formObject, key, arrayPath, tpldata) {
  var value = null;
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  formData.getValue = function (key) {
    return getInitialValue(self.ownerTree.formDesc, key, self.arrayPath, formData);
  };
  ...
  if (formElement.key) {
    ...
    if (!formElement.type) {
      if ((schemaElement.type === 'number') &&
        (_(['utc-sec', 'utc-millisec']).include(schemaElement.format))) {
