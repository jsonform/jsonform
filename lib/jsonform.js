...
jsonform.defaultTemplates = {
  ...
  'file-transloadit': jsonform.defaultFieldTemplate('<input type="hidden" name="params" value=\'<%= (new Option(JSON.stringify(elt.transloaditParams))).innerHTML %>\'><input type="file" name="<%= elt.name %>-transloaditfile" /><input type="hidden" id="<%= elt.id %>" name="<%= elt.name %>" />'),
...
var buildOneElement = function(elt,options) {
  ...
  if (elt.onClick) {
    eventhandlers.push(["#"+elt.id,"click",elt.onClick]);
  }
  if (elt.onInsert) {
    eventhandlers.push(["#"+elt.id,"insert",elt.onInsert]);
  }
...
$.fn.jsonForm = function(options) {
  ...
  this.data("jsonform-schema",options.schema);

  this.unbind((options.submitEvent||'submit')+'.jsonform');
  ...
  this.bind((options.submitEvent||'submit')+'.jsonform', function(event) {
    var value = jsonform.getFormValue(this);

    if (options.onSubmit) {
      if (!options.onSubmit(null,value)) {
