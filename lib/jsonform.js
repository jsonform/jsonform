...
jsonform.elementTypes = {
  ...
  'array': {
    ...
    'onInsert': function (evt, node) {
      ...
      if(node.formElement.draggable === undefined || node.formElement.draggable==="true"){
        if ($(node.el).sortable) {
          $('> ul', $nodeid).sortable();
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(node.children, function (child, idx) {
        ...
        tabs += '<li data-idx="' + idx + '"' +
          ((idx === 0) ? ' class="active"' : '') +
          '><a class="draggable tab" data-toggle="tab" val="'+escapeHTML(title)+'">' +
          escapeHTML(title) +
          '</a></li>';
      });
      data.tabs = tabs;
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  if (this.view && this.view.inputfield && this.schemaElement) {
    ...
    if (values) {
      ...
      // the value is not provided in the values section
      else {
        // is the default set in the schemaElement (which we have)
        if(isSet(this.schemaElement['default'])){
          // then get it as the field value
          this.value = this.schemaElement['default']
          // does it contain a template variable?
          if (this.value.includes('{{')) {
            // then apply the templating
...
formNode.prototype.getPreviousNumberOfItems = function (values, arrayPath) {
  ...
  if (this.view.inputfield && this.schemaElement) {
    ...
    if(typeof arrayValue === 'string'){
      ...
      // and its not an array we need to nest info
      return 1;
    }
...
formNode.prototype.generate = function () {
  ...
  if (this.parentNode && this.parentNode.view &&
    this.parentNode.view.childTemplate) {
    template = this.parentNode.view.childTemplate(template, (this.parentNode.formElement.draggable === undefined ? true : this.parentNode.formElement.draggable));
  }
