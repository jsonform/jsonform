 ...
 *
 * Input fields that are in non visible tabs are automatically disabled
 * in order not to appear in the submitted form. That's on purpose, as tabs
 * are meant to convey an alternative (and not a sequence of steps).
 *
 * Code from bootstrap-tabs.js, updated to disable input fields in non visible
 * tabs and to act as a regular function instead of as a jQuery plug-in.
 *
 * @function
...
var initializeTabs = function (tabs) {
  ...
  tabs.each(function () {
    $(this).delegate('ul[data-tabs] li > a, ul[data-pills] > li > a', 'click', tabClicked);
    $('ul[data-tabs] li > a, ul[data-pills] > li > a').first().click();
  });
};
  ...
  'selectfieldset': {
    'template': '<fieldset>' +
      ...
      '<% _.each(elt.items, function(item, idx) {' +
        ...
        '<a href="#<%= item.id %>"><%= item.legend %></a></li><%' +
      '});' +
      '%></ul>' +
...
var getObjKey = function (obj, key, ignoreArrays) {
  ...
  for (var i = 0; i < keyparts.length; i++) {
    ...
    subkey = keyparts[i];
    if (arrayMatch = subkey.match(reArray)) {
...
formFragment.prototype.buildOneElement = function (elt,options) {
  ...
  if (elt.key) {
    ...
    // Tabs need IDs on children, first tab is the active one
  ...
  if (elt.type === 'array') {
    ...
    var genSubElementFormWithValue = function(v,elt,i) {
      ...
      if (elt.key) {
        ...
        var flagArrayItems = function (element) {
          ...
          for (var i = 0; i < element.items.length; i++) {
            currItem = element.items[i];
            if (_.isString(currItem)) {
              element.items[i] = currItem.replace(/\[\]/, '[XXXkeyXXX]');
            }
            else if (currItem.key) {
              element.items[i].key = currItem.key.replace(/\[\]/, '[XXXkeyXXX]');
            }
            flagArrayItems(currItem);
          };
        };
        flagArrayItems(elt);
      ...
      if (i>=0) {
        if (elt.key) {
          elt.itemshtml += "<li>" +
            subForm.html.replace(/XXXkeyXXX/g, elt.key + "[" + i + "]").replace(/XXXidxXXX/g, (i+1)) +
            "</li>";
        }
        else {
          elt.itemshtml += "<li>" +
            subForm.html.replace(/XXXkeyXXX/g, i).replace(/XXXidxXXX/g, (i+1)) +
            "</li>";
        }
      }
