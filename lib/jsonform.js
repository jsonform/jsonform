...
jsonform.fieldTemplate = function(inner) {
  return '<div class="control-group jsonform-error-<%= elt.keydash %>">'+
...
jsonform.elementTypes = {
  ...
  'ace':{
    'template':'<div id="<%= elt.id %>" style="position:relative;height:<%= elt.height %>;"><div id="<%= elt.id %>__ace" style="width:<%= elt.width %>;height:<%= elt.height %>;"><%= elt.value %></div><input type="hidden" name="<%= elt.name %>" id="<%= elt.id %>__hidden" value="<%= escape(elt.value) %>"/></div>',
    'fieldtemplate':true,
    'handlers':{
  ...
  'checkboxesitem':{
    ...
    // inserted to avoid float clearing problems
    'template':'<label class="checkbox"><input type="checkbox" <% if (elt.value) { %> checked="checked" <% } %> name="<%= elt.name %>" value="1"><span><%= elt.title || schema.title || " " %></span></label>'
  },
  'checkboxes':{
  ...
  'fieldset':{
    'template': '<fieldset class="control-group jsonform-error-<%= elt.keydash %> <% if (elt.expandable) { %>expandable<% } %>" ' +
      '<% if (elt.id) { %> id="<%= elt.id %>"<% } %>' +
      '>' +
  ...
  'submit':{
    'template':'<input type="submit" id="<%= elt.id %>" class="btn btn-primary" value="<%= elt.value %>"/>'
  },
  'button':{
    'template':'<button id="<%= elt.id %>" class="btn"><%= elt.title %></button>'
  },
  'actions':{
    'template':'<div class="form-actions"><%= elt.itemshtml %></div>'
  },
  'hidden':{
