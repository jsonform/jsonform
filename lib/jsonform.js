...
jsonform.elementTypes = {
  ...
  'checkboxesitem':{
    ...
    // inserted to avoid float clearing problems
    'template':'<label class="checkbox <%= elt.htmlClass?elt.htmlClass:"" %>"><input type="checkbox" <% if (value) { %> checked="checked" <% } %> name="<%= node.name %>" value="1"><span><%= node.title || schema.title || " " %></span></label>',
    'inputfield': true
  },
  'checkboxes':{
    'template':'<%= children %>',
...
formNode.prototype.getFormValues = function (updateArrayPath) {
  ...
  for (var i = 0; i < formArray.length; i++) {
    ...
    var arrayMatch = name.match(reArraySingle);

    // Handle multiple checkboxes separately as the idea is to generate
    // an array that contains the list of enumeration items that the user
    // selected.
    // TODO: this probably does not work for multiple checkboxes nested
    // within an array. Adapt the code.
    if (arrayMatch && eltSchema._jsonform_checkboxes_as_array) {
      name = name.replace(reArraySingle, '');
