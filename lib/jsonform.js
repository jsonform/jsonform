...
jsonform.elementTypes = {
  ...
  'checkboxes':{
    'template':'<ul class="inputs-list" id="<%= elt.id %>"><% _.each(elt.options, function(key, val) { %> <li><label><input type="checkbox" <% if (elt.value == key) { %> checked="checked" <% } %> name="<%= elt.name %>" value="<%= key %>"><span><%= key %></span></label></li> <% }); %></ul>',
    'fieldtemplate':true
  },
  'array':{
