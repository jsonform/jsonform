    ...
    'childTemplate': function (inner, node) {
      if (!node.isReadOnly() && $('').sortable) {
        ...
        return '<li data-idx="<%= node.childPos %>">' +
          ...
          ' <a href="#" class="_jsonform-array-item-delete"><i class="<%= cls.iconClassPrefix %>-remove" style="float:right;" title="Remove item"></i></a>' +
          inner +
          '</li>';
      }
      else {
...
formTree.prototype.initialize = function (formDesc) {
  ...
  this.defaultClasses = jsonform.defaultClasses = getDefaultClasses(this.formDesc.isBootstrap2 || jsonform.isBootstrap2);
...
formTree.prototype.buildTree = function () {
  ...
  _.each(this.formDesc.form, function (formElement) {
    if (formElement === '*') {
      if(_.isUndefined(this.formDesc.schema.propertiesOrder)){
        ...
        _.each(this.formDesc.schema.propertiesOrder, function (key) {
          var element = this.formDesc.schema.properties[key]
  ...
  if (formElement.key) {
    ...
    if (!formElement.type) {
      if ((schemaElement.type === 'string') &&
        ...
        (schemaElement.format === 'yaml')) {
        formElement.type = "ace"
        formElement.aceMode = "yaml"
        formElement.aceTheme = "monokai"
        formElement.width = "100%"
        //formElement.height = "500px"
      } else if ((schemaElement.type === 'string') &&
        (schemaElement.format === 'jsonschema')) {
        formElement.type = "ace"
        formElement.aceMode = "yaml"
        formElement.aceTheme = "monokai"
        formElement.width = "100%"
        ...
        (schemaElement.format === 'javascript')) {
        formElement.type = "ace"
        formElement.aceMode = "javascript"
        formElement.aceTheme = "monokai"
        formElement.width = "100%"
        //formElement.height = "500px"
      ...
      } else if (schemaElement.type === 'array' && schemaElement.items.type != "string") {
        formElement.type = 'tabarray';
        ...
        if(!_.isUndefined(schemaElement['options'])){
          formElement.options = schemaElement['options'];
        }
      } else {
    ...
    if (schemaElement.type === 'object') {
      if(_.isUndefined(schemaElement.propertiesOrder)){
        ...
        _.each(schemaElement.propertiesOrder, function (propName) {
          var prop = schemaElement.properties[propName];
...
//        console.log(errormarkerclass);
      ...
      $node.addClass(jsonform.defaultClasses.groupMarkClassPrefix + errorType);
  ...
  $errorInvisiblePanels.each(function() {
    var $this = $(this);
    $errorTabs = $errorTabs.add($this.closest('.tabbable').find('> .nav > li').eq($this.index()).addClass(jsonform.defaultClasses.groupMarkClassPrefix + 'error'));
  });
