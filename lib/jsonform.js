...
jsonform.elementTypes = {
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<a class="btn" data-toggle="dropdown" href="#">' +
        '<% if (node.value) { %><img src="<%= prefix %><%= node.value %>" alt="" width="<%= width %>" height="<%= height %>" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu navbar" id="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(elt.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } if (key instanceof Object) { %><a class="btn"><img src="<%= prefix %><%= key.value %>" alt="<%= key.title %>" width="<%= width %>" height="<%= height %>" /></a><% } else { %><a class="btn"><img src="<%= prefix %><%= key %>" alt="" width="<%= width %>" height="<%= height %>" /></a><% } }); %>' +
        '</div>' +
      '</div>' +
      '</div>' +
      '</div>',
    ...
    'onBeforeRender': function (data, node) {
      ...
      data.buttonTitle = elt.imageSelectorTitle || 'Choose one...';
    ...
    'onInsert': function (evt, node) {
      $(node.el).on('click', '.dropdown-menu a', function (evt) {
        ...
        $(node.el).find('a[data-toggle="dropdown"]').html('<img src="' +
          prefix + value + '" alt="" width="' + width + '"' +
          'height="' + height + '" />');
      });
    }
  },
  ...
  'tabarray': {
    'template': '<div id="<%= id %>"><div class="tabbable tabs-left">' +
      ...
      '</div>' +
      '<a href="#" class="btn _jsonform-array-addmore"><i class="icon-plus-sign" title="Add new"></i></a> ' +
      '<a href="#" class="btn _jsonform-array-deleteitem"><i class="icon-remove-sign" title="Delete item"></i></a></div>',
    'fieldtemplate': true,
    'array': true,
    'childTemplate': function (inner) {
