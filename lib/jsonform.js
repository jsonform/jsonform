...
jsonform.elementTypes = {
  ...
  'textarea':{
    'template':'<textarea id="<%= elt.id %>" name="<%= elt.name %>" style="height:<%= elt.height || "150px" %>;width:<%= elt.width || "100%" %>;"><%= elt.value %></textarea>',
    'fieldtemplate':true
  },
  'wysihtml5':{
  ...
  'file-transloadit':{
    'template':'<% if (elt.value && (elt.value.type||elt.value.url)) { %>'+fileDisplayTemplate+'<% } %><input id="_transloadit_<%= elt.id %>" type="file" name="_transloadit_<%= elt.name %>" /><input type="hidden" id="<%= elt.id %>" name="<%= elt.name %>" value=\'<%= escape(JSON.stringify(elt.value)) %>\' />',
    'fieldtemplate':true
  },
  'select':{
  ...
  'tabarray': {
    'template': '<div class="tabbable tabs-left" id="<%= elt.id %>">' +
...
formFragment.prototype.buildOneElement = function (elt,options) {
  ...
  if (elt.key) {
    ...
    if (!elt.id) {
      elt.id = escapeSelector(options.prefix) + "-elt-" + elt.key;
    }
    ...
    _.each(elt.items, function (item) {
      if (!item.id) {
        item.id = escapeSelector(options.prefix) + "-elt-counter-" + _.uniqueId();
      }
      if (item === activeItem) {
        item.active = true;
      }
  ...
  if ((elt.type === 'array') || (elt.type === 'tabarray')) {
    ...
    var genSubElementFormWithValue = function(v,elt,idx) {
      ...
      if (elt.key) {
        ...
        elt.id = escapeSelector(options.prefix) + "-elt-counter-" + _.uniqueId();
        elt.iddot = escapeSelector(elt.id||"");
      ...
      if ((idx !== 'XXXkeyXXX') && (valueFound || (idx === 0))) {
        if (elt.type !== "tabarray") {
          elt.itemshtml += "<li>";
        }
        elt.itemshtml += subForm.html
          .replace(/XXXkeyXXX/g, (elt.key ? elt.key + "[" + idx + "]" : idx))
          .replace(/XXXidxXXX/g, (idx+1))
          .replace(/XXXnameXXX/g, (v ? v.name : ''));
        ...
        elt.itemstabs += "<li" +
          (idx === 0 ? " class='active'" : "") +
          "><a data-toggle='tab'>";
        if (elt.items && elt.items[0] && elt.items[0].legend) {
          elt.itemstabs += elt.items[0].legend
            .replace(/XXXkeyXXX/g, elt.key + "[" + idx + "]")
            .replace(/XXXidxXXX/g, (idx+1))
            .replace(/XXXnameXXX/g, (v ? v.name : ''));
        }
        else {
          elt.itemstabs += "Item " + (idx+1);
        }
        elt.itemstabs += "</a></li>";
    ...
    elt.itemstabstpl = "<li><a data-toggle='tab'>" +
      ((elt && elt.items && elt.items[0] && elt.items[0].legend) ?
        elt.items[0].legend :
        "Item XXXidxXXX") +
      "</a></li>";
    ...
    this.eventhandlers.push(["#"+elt.id,"insert",function() {
      ...
      var addMore = function(newNo) {
        ...
        if (newNo==-1) {
          container = $("#" + elt.iddot);
          if (elt.type !== "tabarray") {
            tabcontainer = container.children('ul');
            itemcontainer = container;
            itemshtml = "<li>";
          }
          else {
            tabcontainer = container.children('ul');
            itemcontainer = container.children('div');
          }
          newNo = itemcontainer.children().size();
          itemshtml += elt.itemshtmltpl
            .replace(/XXXkeyXXX/g, (elt.key ? elt.key+ "[" + newNo + "]" : newNo))
            .replace(/XXXidxXXX/g, (newNo+1));
          if (elt.type !== "tabarray") {
            itemshtml += "</li>";
          }

          itemstabs = elt.itemstabstpl
            .replace(/XXXkeyXXX/g, (elt.key ? elt.key+ "[" + newNo + "]" : newNo))
            .replace(/XXXidxXXX/g, (newNo+1));

          $(itemcontainer).append(itemshtml);
          $(tabcontainer).append(itemstabs);
          $(tabcontainer).children().last().click();
        }
      ...
      var deleteItem = function (idx) {
        // TODO: IDs of the input fields that follow the field removed
        // must be shifted. Or these fields need to be re-generated
        // (but without triggering change events, sigh).
        var selected = false;
        var selectIdx = -1;
        if (elt.type !== "tabarray") {
          $("#"+elt.iddot).children().eq(idx).remove();
        }
        else {
          selected = $("#"+elt.iddot).children('ul').children().eq(idx).hasClass('active');
          $("#"+elt.iddot).children('ul').children().eq(idx).remove();
          $("#"+elt.iddot).children('div').children().eq(idx).remove();
          if (selected) {
            if (idx === -1) selectIdx = -1;
            else if (idx === 0) selectIdx = 1;
            else selectIdx = idx - 1;
            $("#"+elt.iddot).children('ul').children().eq(selectIdx).click();
          }
        }
      };
      ...
      $("a._jsonform-array-moveup",$("#"+elt.iddot).parent()).click(function(evt) {
        evt.preventDefault();
      });

      $("a._jsonform-array-movedown",$("#"+elt.iddot).parent()).click(function(evt) {
        evt.preventDefault();
      });

      $("a._jsonform-array-deleteitem",$("#"+elt.iddot).parent()).click(function(evt) {
        evt.preventDefault();
      });
    ...
    elt.itemshtml = subForm.html;
    /*
    if (elt.type === 'tabfieldset') {
      // Add "move up", "move down", "delete" links
      elt.itemsbuttons = '<span class="_jsonform-array-buttons">' +
        '<a href="#" class="_jsonform-array-moveup">Move up</a> | ' +
        '<a href="#" class="_jsonform-array-movedown">Move down</a> | ' +
        '<a href="#" class="_jsonform-array-deleteitem">Delete item' +
        '</span>';
    }
    */
    subForm.html="";
...
$.fn.jsonForm = function(options) {
  ...
  options.prefix = options.prefix || "jsonform-" + _.uniqueId();
