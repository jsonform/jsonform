...
jsonform.defaultTemplates = {
  ...
  'file-transloadit': jsonform.defaultFieldTemplate('<input type="hidden" name="params" value=\'<%= (new Option(JSON.stringify(elt.transloaditParams))).innerHTML %>\'><input type="file" name="transloaditfile-<%= elt.name %>" />'), /*<input type="hidden" id="<%= elt.id %>" name="<%= elt.name %>" />*/
