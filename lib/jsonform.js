...
/**
 * Returns the default value that a field identified by its key
 * should take.
 *
 * @function
 * @param {Object} formObject The JSON Form object
 * @param {String} key The generic key path (e.g. foo[].bar.baz[])
 * @param {Array(Number)} arrayPath The array path
 */
var getInitialValue = function (formObject, key, tpldata) {
  var value = null;

  // Complete template data for template function
  // TODO: extract idx from last array index in key
  tpldata = tpldata || {};
  tpldata.idx = tpldata.idx || 1;
  tpldata.value = tpldata.value || '';
  tpldata.getValue = tpldata.getValue || function (key) {
    return getInitialValue(formObject, key, tpldata);
  };

  // Helper function that returns the form element that explicitly
  // references the given key in the schema.
  var genericKey = key.replace(reArray, '[]');
  ...
  var formElement = getFormElement(formObject.form || [], genericKey);

  if (formObject.value) {
    // If values were previously submitted, use them directly if defined
    value = getObjKey(formObject.value, key);
  }
  if ((typeof value === 'undefined') || (value === null)) {
    if (formElement && (typeof formElement['value'] !== 'undefined')) {
      ...
      var schemaElement = getSchemaKey(
        formObject.schema.properties, genericKey);
      if (schemaElement) {
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  formData.getValue = function (key) {
    return getInitialValue(self.ownerTree.formDesc, key, formData);
  };
