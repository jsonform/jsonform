...
var initializeTabs = function (tabs) {
  ...
  var optionSelected = function (e) {
    ...
    $target = $(this).parent().parent().parent().find('.tab-content [data-idx=' + targetIdx + ']');
...
jsonform.fieldTemplate = function(inner) {
  return '<div class="control-group jsonform-error-<%= keydash %> <%= elt.htmlClass?elt.htmlClass:"" %>">'+
          '<label class="control-label" for="<%= id %>"><% if (!elt.notitle) { %><%= node.title %><% } %></label>'+
...
jsonform.elementTypes = {
  ...
  'array': {
    'template': '<ul id="<%= id %>" class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>',
    'fieldtemplate': true,
    'array': true,
    ...
    'onInsert': function (evt, node) {
      ...
      $(node.el).on('click', 'a._jsonform-array-additem', function (evt) {
        evt.preventDefault();
        var idx = $(evt.target).parent().parent().attr('data-idx');
        idx = parseInt(idx, 10) + 1;
        node.insertArrayItem(idx, $('#' + escapeSelector(node.id)).get(0));
      });

      $(node.el).on('click', 'a._jsonform-array-moveup', function (evt) {
        evt.preventDefault();
        var idx = $(evt.target).parent().parent().attr('data-idx');
        idx = parseInt(idx, 10);
        if (idx <= 0) return;
        switchNodes(idx, idx-1);
      });

      $(node.el).on('click', 'a._jsonform-array-movedown', function (evt) {
        evt.preventDefault();
        var idx = $(evt.target).parent().parent().attr('data-idx');
        idx = parseInt(idx, 10);
      ...
      $(node.el).on('click', 'a._jsonform-array-deleteitem', function (evt) {
        evt.preventDefault();
        var idx = $(evt.target).parent().parent().attr('data-idx');
        idx = parseInt(idx, 10);
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      ...
      var updateTabs = function (selIdx) {
        var tabs = '';
        if (selIdx === undefined) {
          selIdx = $('.nav-tabs .active', node.el).attr('data-idx');
          if (selIdx) {
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  if (this.formElement) {
    ...
    else if (this.parentNode && this.parentNode.formElement &&
      jsonform.elementTypes[this.parentNode.formElement.type].array) {
  ...
  if (this.formElement && this.formElement.valueInLegend) {
    ...
    while (node) {
      if (node.parentNode &&
        node.parentNode.formElement &&
        jsonform.elementTypes[node.parentNode.formElement.type].array) {
  ...
  // as before)
  // TODO: update code that relies on onElementValue as it now receives a formNode
  if (this.formElement) {
    if (this.ownerTree.formDesc.onElementValue) {
      this.ownerTree.formDesc.onElementValue(this);
    }
  }
};
...
formNode.prototype.generate = function () {
  ...
  if (this.formElement && this.formElement.template) {
  ...
  if (view.onBeforeRender) {
    view.onBeforeRender(data, this);
  }
...
formNode.prototype.enhance = function () {
  ...
  if (this.formElement) {
    ...
    view = jsonform.elementTypes[this.formElement.type];
    if (view.onInsert) {
    ...
    if (this.legendChild && this.legendChild.formElement) {
      $(this.legendChild.el).bind('change', function (evt) {
