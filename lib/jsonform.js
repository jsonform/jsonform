  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
        '<% if (node.value) { %><img src="<%= prefix %><%= node.value %>" alt="" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu navbar" id="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(elt.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" style="max-width:<%= width %>px;max-height:<%= height %>px"><% if (key instanceof Object) { %><img src="<%= prefix %><%= key.value %>" alt="<%= key.title %>" /></a><% } else { %><img src="<%= prefix %><%= key %>" alt="" /><% } %></a> <% }); %>' +
        '</div>' +
      '</div>' +
      '</div>' +
      '</div>',
