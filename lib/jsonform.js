...
jsonform.elementTypes = {
  ...
  'selectfieldset': {
    'template': '<fieldset id="<%= node.id %>" class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %>">' +
    ...
    'onBeforeRender': function (data, node) {
      ...
      var elt = node.formElement;
      var tabsType = elt.tabsType || 'select';
      var tabs = null;

      switch (tabsType) {
        case 'select':
          tabs = '<select class="nav">';
          ...
          break;
        case 'tabs':
          tabs = '<ul class="nav nav-tabs jsonform-alternative">';
          _.each(node.children, function (child) {
            tabs += '<li data-idx="' + child.childPos + '"' +
              (child.active ? ' class="active"' : '') +
              '>' +
              '<a href="#">' + escapeHTML(child.legend ||
                child.title ||
                ('Option ' + (child.childPos+1))) +
              '</a>';
          });
          ...
          break;
        case 'pills':
          tabs = '<ul class="nav nav-pills jsonform-alternative">';
          _.each(node.children, function (child) {
            tabs += '<li data-idx="' + child.childPos + '"' +
              (child.active ? ' class="active"' : '') +
              '>' +
    ...
    'onInsert': function (evt, node) {
      ...
      $(node.el).find('input[type="radio"]').on('change', function (evt) {
        ...
        if (option.setvalue) {
          ...
          $(node.parentNode.el).find('input[type="hidden"]').val(option.setvalue);
        }
        if (option.next) {
