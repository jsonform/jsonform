...
jsonform.elementTypes = {
  ...
  'imageselect': {
    'template': '<div>' +
      ...
      '<div class="dropdown">' +
      '<a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" data-toggle="dropdown" href="#">' +
        '<% if (node.value) { %><img src="<%= prefix %><%= node.value %>" alt="" width="<%= width %>" height="<%= height %>" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu navbar" id="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(elt.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>"><% if (key instanceof Object) { %><img src="<%= prefix %><%= key.value %>" alt="<%= key.title %>" width="<%= width %>" height="<%= height %>" /></a><% } else { %><img src="<%= prefix %><%= key %>" alt="" width="<%= width %>" height="<%= height %>" /><% } %></a> <% }); %>' +
        '</div>' +
      '</div>' +
      '</div>' +
      '</div>',
