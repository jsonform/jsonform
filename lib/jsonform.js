...
(function($, _, JSON, global) {
  ...
  // TODO: tempateSettings can be passed as parameters to "_.template",
  // why not use that directly to avoid relying on a specific version of
  // the function?
...
jsonform.elementTypes = {
  ...
  'wysihtml5':{
    ...
    'onInsert': function (evt, node) {
      var setup = function () {
        ...
        $('#' + escapeSelector(node.id)).wysihtml5({
          "html": true,
  ...
  'imageselect': {
    'template': '<div>' +
      '<input type="hidden" name="<%= node.name %>" id="<%= node.id %>" value="<%= node.value %>" />' +
      '<div class="dropdown">' +
      '<a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" data-toggle="dropdown" href="#"<% if (node.value) { %> style="max-width:<%= width %>px;max-height:<%= height %>px"<% } %>>' +
        '<% if (node.value) { %><img src="<% if (!node.value.match(/^https?:/)) { %><%= prefix %><% } %><%= node.value %>" alt="" /><% } else { %><%= buttonTitle %><% } %>' +
      '</a>' +
      '<div class="dropdown-menu navbar" id="<%= node.id %>_dropdown">' +
        '<div>' +
        '<% _.each(elt.options, function(key, idx) { if ((idx > 0) && ((idx % columns) === 0)) { %></div><div><% } %><a class="btn<% if (buttonClass) { %> <%= buttonClass %><% } %>" style="max-width:<%= width %>px;max-height:<%= height %>px"><% if (key instanceof Object) { %><img src="<% if (!key.value.match(/^https?:/)) { %><%= prefix %><% } %><%= key.value %>" alt="<%= key.title %>" /></a><% } else { %><img src="<% if (!key.match(/^https?:/)) { %><%= prefix %><% } %><%= key %>" alt="" /><% } %></a> <% }); %>' +
        '</div>' +
    ...
    'onBeforeRender': function (data, node) {
      ...
      data.buttonTitle = elt.imageSelectorTitle || 'Choose...';
      data.prefix = elt.imagePrefix || '';
      data.width = elt.imageWidth || 32;
      data.height = elt.imageHeight || 32;
      data.buttonClass = elt.imageButtonClass || false;
      if (elt.options.length > maxColumns) {
        nbRows = Math.ceil(elt.options.length / maxColumns);
        data.columns = Math.ceil(elt.options.length / nbRows);
      }
      else {
      ...
      $(node.el).on('click', '.dropdown-menu a', function (evt) {
        ...
        if (value) {
          ...
          $(node.el).find('a[data-toggle="dropdown"]')
            ...
            .html('<img src="' + (!value.match(/^https?:/) ? prefix : '') + value + '" alt="" />');
        }
        else {
          $(node.el).find('input').attr('value', '');
          $(node.el).find('a[data-toggle="dropdown"]')
            .removeAttr('style')
            .html(elt.imageSelectorTitle || 'Choose...');
        }
      });
    }
  },
  ...
  'checkboxes':{
    ...
    'onBeforeRender': function (data, node) {
      ...
      var template = '<label class="checkbox">' +
        '<input type="checkbox" <% if (value) { %> checked="checked" <% } %> name="<%= name %>" value="1"><span><%= title %></span></label>';
  ...
  'array': {
    ...
    'onInsert': function (evt, node) {
      ...
      var switchNodes = function (fromIdx, toIdx) {
        ...
        node.children[fromIdx].render($('#' + escapeSelector(node.id)).get(0));
        node.children[toIdx].render($('#' + escapeSelector(node.id)).get(0));
      };

      $(node.el).on('click', 'a._jsonform-array-addmore', function (evt) {
        evt.preventDefault();
        var idx = node.children.length;
        node.insertArrayItem(idx, $('#' + escapeSelector(node.id) + ' ul').get(0));
      });

      $(node.el).on('click', 'a._jsonform-array-deletelast', function (evt) {
        evt.preventDefault();
  ...
  'tabarray': {
    ...
    'onInsert': function (evt, node) {
      ...
      var updateTabs = function (selIdx) {
        var tabs = '';
        if (selIdx === undefined) {
          selIdx = $('.nav-tabs .active', node.el).data('idx');
          if (selIdx) {
        ...
        $('.nav-tabs', node.el).html(tabs);
        $('.nav-tabs [data-toggle="tab"]', node.el).eq(selIdx).click();
      };

      $('a._jsonform-array-deleteitem', $(node.el).parent()).click(function (evt) {
      ...
      if ($(node.el).sortable) {
        $(node.el).find('.nav-tabs').sortable({ containment: node.el });
  ...
  'submit':{
    'template':'<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-primary <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"/>'
  },
  'button':{
    'template':'<button id="<%= id %>" class="btn <%= elt.htmlClass?elt.htmlClass:"" %>"><%= node.title %></button>'
  },
  'actions':{
  ...
  'selectfieldset': {
    ...
    'onBeforeRender': function (data, node) {
      ...
      _.each(node.children, function (child) {
        tabs += '<option value="' + child.childPos + '"' +
          (child.active ? ' class="active"' : '') +
          '>' +
          escapeHTML(child.legend ||
            child.title ||
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  if (this.formElement) {
    ...
    // Same for the title although that's probably meaningless.
    // Also replace {{idx}} by the index of the child in its wrapping array
    // (starting at 1 since that's more human-friendly than a zero-based index)
    this.title = applyArrayPath(this.formElement.title, this.arrayPath);
    if (this.title) {
      this.title = this.title.replace(/(XXX|\{\{)idx(XXX|\}\})/g,
        (this.arrayPath.length > 0) ?
          this.arrayPath[this.arrayPath.length-1] + 1 :
          this.childPos + 1);
    }

    // Same for the legend although, again, that's probably meaningless.
    // Also replace {{idx}} by the index of the child in its wrapping array
    // (starting at 1 since that's more human-friendly than a zero-based index)
    this.legend = applyArrayPath(this.formElement.legend, this.arrayPath);
    if (this.legend) {
      this.legend = this.legend.replace(/(XXX|\{\{)idx(XXX|\}\})/g,
        (this.arrayPath.length > 0) ?
          this.arrayPath[this.arrayPath.length-1] + 1 :
          this.childPos + 1);
    }
    ...
    this.value = applyArrayPath(this.formElement.value, this.arrayPath);
    if (this.value) {
      this.value = this.value.replace(/(XXX|\{\{)idx(XXX|\}\})/g,
        (this.arrayPath.length > 0) ?
          this.arrayPath[this.arrayPath.length-1] + 1 :
          this.childPos + 1);
    }
    ...
    while (node) {
      if (node.parentNode &&
        ...
        if (node.legend) {
          ...
          node.legend =  node.legend.replace(/\{\{value\}\}/, this.value || "");
          break;
        }
      }
...
formNode.prototype.enhance = function () {
  ...
  if (this.formElement) {
    ...
    if (this.el) {
      ...
      if (handler) {
        $(this.el).bind('change', handler);
      }
      handler = this.onClick ||
        this.view.onChange ||
        this.formElement.onClick;
      if (handler) {
    ...
    if (this.legendChild && this.legendChild.formElement) {
      $(this.legendChild.el).bind('keyup', function (evt) {
        if (node.formElement && node.formElement.legend && node.parentNode) {
          ...
          node.legend = node.legend.replace(/(XXX|\{\{)idx(XXX|\}\})/g,
            (node.arrayPath.length > 0) ?
              node.arrayPath[node.arrayPath.length-1] + 1 :
              node.childPos + 1);
          node.legend = node.legend.replace(/\{\{value\}\}/, $(evt.target).val());
