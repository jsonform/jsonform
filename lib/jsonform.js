...
var walkElementsInSchema = function(schema, options,prefix) {
  ...
  _.each(schema.properties, function(prop, key) {
    if (prop.properties) {
      str += walkElementsInSchema(prop,options,prefix+key+".");
    } else {
      str += buildOneElement({'key':prefix+key},options);
    }
  });
  return str;
};
...
var buildForm = function(options) {
  ...
  _.each(options.form, function(elt) {
    ...
    if (elt === '*') {
      str += walkElementsInSchema(options.schema,options,"");
      return;
    }
