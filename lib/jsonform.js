...
jsonform.elementTypes = {
  ...
  'color':{
    ...
    'onInsert': function(evt, node) {
      $("#" + escapeSelector(node.id)).spectrum({
        preferredFormat: "hex",
        showInput: true
      });
    }
  ...
  'checkbox':{
    ...
    'template':'<label class="checkbox"><input type="checkbox" id="<%= id %>" name="<%= node.name %>" value="1" <% if (value) {%>checked<% } %> /><span><%= node.inlinetitle || " " %></span></label>',
    'fieldtemplate': true,
    'inputfield': true,
  ...
  'select':{
    'template':'<select name="<%= node.name %>" id="<%= id %>"> <% _.each(node.options, function(key, val) { if(key instanceof Object) { if (value === key.value) { %> <option selected value="<%= key.value %>"><%= key.title %></option> <% } else { %> <option value="<%= key.value %>"><%= key.title %></option> <% }} else { if (value === key) { %> <option selected value="<%= key %>"><%= key %></option> <% } else { %><option value="<%= key %>"><%= key %></option> <% }}}); %> </select>',
    'fieldtemplate': true,
    'inputfield': true
  },
  'imageselect': {
  ...
  'radios':{
    'template':'<div id="<%= node.id %>"><% _.each(node.options, function(key, val) { %><label class="radio"><input type="radio" <% if (((key instanceof Object) && (value === key.value)) || (value === key)) { %> checked="checked" <% } %> name="<%= node.name %>" value="<%= (key instanceof Object ? key.value : key) %>"><span><%= (key instanceof Object ? key.title : key) %></span></label><% }); %></div>',
    'fieldtemplate': true,
    'inputfield': true
  },
  'checkboxes':{
...
formNode.prototype.computeInitialValues = function (values, ignoreDefaultValues) {
  ...
  // TODO: handle recursion, the default value could use {{value key}}
  // notation as well (well, not sure there's a valid use case for this)
  formData.jsonform = {};
  formData.jsonform.getValue = function (key) {
    var value = null;

    // Helper function that returns the form element that explicitly
    // reference the given key in the schema.
    var getFormElement = function (elements) {
      if (!elements || !elements.length) return null;
      return _.find(elements, function (elt) {
        if (elt === key) return { key: elt };
        if (_.isString(elt)) return null;
        if (elt.key === key) return elt;
        if (elt.items) return getFormElement(elt.items);
      });
    };
    var formElement = getFormElement(self.ownerTree.formDesc.form);

    if (values) {
      // If values were previously submitted, use them directly,
      // but we may need to amend it for enumerations
      value = getObjKey(values, applyArrayPath(key, self.arrayPath));
    }
    else if (formElement && (typeof formElement['value'] !== 'undefined')) {
      // Extract the definition of the form field associated with
      // the key as it may override the schema's default value
      value = _.template(formElement['value'], formData, valueTemplateSettings);
    }
    else {
      // Simply extract the default value from the schema
      var schemaElement = getSchemaKey(
        self.ownerTree.formDesc.schema.properties, key);
      if (schemaElement) {
        value = _.template(schemaElement['default'] || '',
          formData, valueTemplateSettings);
      }
    }

    // Apply titleMap if needed
    if (value && formElement && formElement.titleMap &&
  ...
  if (this.formElement) {
    ...
    _.each([
      ...
      if (_.isString(this.formElement[prop])) {
        if (this.formElement[prop].indexOf('{{value ') !== -1) {
    ...
    if (this.formElement.options) {
      this.options = _.map(this.formElement.options, function (option) {
        ...
        if (_.isObject(option) && option.title) {
          ...
          // preparation here.
          if (option.title.indexOf('{{value ') !== -1) {
