...
jsonform.elementTypes = {
  ...
  'file-hosted-public':{
    'template':'<div><% if (value && (value.type||value.url)) { %>'+fileDisplayTemplate+'<% } %><input id="_transloadit_<%= id %>" type="file" name="<%= transloaditname %>" /><input data-transloadit-name="_transloadit_<%= transloaditname %>" type="hidden" id="<%= id %>" name="<%= node.name %>" value=\'<%= escape(JSON.stringify(node.value)) %>\' /></div>',
    'fieldtemplate': true,
    'inputfield': true,
    'getElement': function (el) {
      return $(el).parent().get(0);
  ...
  'file-transloadit':{
    'template':'<div><% if (value && (value.type||value.url)) { %>'+fileDisplayTemplate+'<% } %><input id="_transloadit_<%= id %>" type="file" name="_transloadit_<%= node.name %>" /><input type="hidden" id="<%= id %>" name="<%= node.name %>" value=\'<%= escape(JSON.stringify(node.value)) %>\' /></div>',
    'fieldtemplate': true,
    'inputfield': true,
    'getElement': function (el) {
      return $(el).parent().get(0);
...
formTree.prototype.submit = function(evt) {
  ...
  if (options.onSubmit && !options.onSubmit(errors,values)) {
    return stopEvent();
  }

  if (errors) return stopEvent();
