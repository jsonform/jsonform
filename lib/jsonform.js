...
jsonform.defaultTemplates = {
  ...
  'password':     jsonform.defaultFieldTemplate('<input class="xlarge" type="password" name="<%= elt.name %>" value="<%= escape(elt.value) %>" id="<%= elt.id %>" />'),
  ...
  'file-transloadit': jsonform.defaultFieldTemplate('<% if (elt.value) { %>'+fileDisplayTemplate+'<% } %><input id="<%= elt.id %>" type="file" name="_transloadit_<%= elt.name %>" /><input type="hidden" id="<%= elt.id %>" name="<%= elt.name %>" value=\'<%= escape(JSON.stringify(elt.value)) %>\' />'),
  ...
  'array':   jsonform.defaultFieldTemplate('<div id="<%= elt.id %>"><%= elt.itemshtml %></div><a href="#" onclick="$(\'#<%= elt.iddot %>\').append(<%= escape(JSON.stringify("<div>"+elt.itemshtmltpl+"</div>")) %>.replace(/XXXkeyXXX/g,\'<%= elt.key %>[\'+$(\'#<%= elt.iddot %>\').children().size()+\']\')); $(this).next().show(); return false;">Add more</a> <a href="#" onclick="$(\'#<%= elt.iddot %>\').children().last().remove(); return false;">Delete last</a>') // on the 2nd <a> :  style="display:none;"
};
...
formFragment.prototype.buildOneElement = function(elt,options) {
  ...
  if (schema.items && schema.type=="array") {
    ...
    var genSubElementFormWithValue = function(v,elt,i) {
      ...
      var subForm = new formFragment();
      subForm.build(suboptions);
      var tpl = subForm.html;
      if (i>=0) elt.itemshtml += "<div>"+tpl.replace(/XXXkeyXXX/g,elt.key+"["+i+"]")+"</div>";
      subForm.html="";
      return {
        "form":subForm,
        "tpl":tpl
    ...
    for (var i=0;i<elt.value.length;i++) {
      this.concat(genSubElementFormWithValue(elt.value[i],elt,i).form);
    }

    //We always need to generate a blank template for being able to add new elements
    elt.itemshtmltpl = genSubElementFormWithValue(undefined,elt,-1).tpl;
  }
